...
optimizer_y1 = optim.SGD(model_y1.parameters(), lr=learning_rate, momentum=0.9)
optimizer_y2 = optim.SGD(model_y2.parameters(), lr=learning_rate, momentum=0.9)

scheduler_y1 = lr_scheduler.StepLR(optimizer_y1, step_size=1000, gamma=0.1)
scheduler_y2 = lr_scheduler.StepLR(optimizer_y2, step_size=1000, gamma=0.1)
...
for epoch in range(num_epochs):
    ...
    for observations, y1 in dataloader_y1:
        ...
        # print("Observation: ", observations, y1)
        counter += 1
    ...
    optimizer_y1.step()
    scheduler_y1.step()
    current_lr = optimizer_y1.param_groups[0]['lr']

    print(f'Epoch [{epoch+1}/{num_epochs}], Loss Y1: {loss.item():.4f}, LR: {current_lr}')
...
# Define a truncation threshold
# threshold = 1e-2
threshold = 0

# Function to truncate small weights to zero
def truncate_weights(model, threshold):
    with torch.no_grad():
        for param in model.parameters():
            param.data = torch.where(torch.abs(param.data) < threshold, torch.tensor(0.0), param.data)

# Truncate weights for both models
truncate_weights(model_y1, threshold)
truncate_weights(model_y2, threshold)
